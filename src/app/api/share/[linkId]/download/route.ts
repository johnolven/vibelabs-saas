...
// GET: Descargar documento por enlace compartido
export async function GET(
  ...
  try {
    ...
    if (!shareLink.allowDownload) {
      return NextResponse.json(
        { error: 'La descarga no está permitida para este enlace' },
        { status: 403 }
      );
    }
    ...
    // Retornar archivo para descarga (attachment, no inline)
    ...
    console.error('Error al descargar documento compartido:', error);
    return NextResponse.json(
      { error: 'Error al descargar documento compartido' },
      { status: 500 }
    );
  }
}
